...
/// @title JobEscrow — one job, one escrowed ASSET payment, released only on verified delivery
/// @notice Buyer's ASSET sits here until the buyer releases it, an arbiter resolves a dispute
/// in the seller's favor, or a timeout auto-releases it — never before. The seller must have
/// credit committed on CreditLine before a job can start, and a dispute resolved against the
/// seller draws that same commitment from the credit pool to the buyer, on top of the escrowed
/// refund.
///
/// This is the one structural change from Tripwire. There, the seller posted 100% of the
/// collateral themselves (SellerBond) and the protocol carried no exposure. Here the seller
/// posts nothing: an A-Pass-gated credit line stands behind them, and the pool's lenders carry
/// the exposure. JobEscrow's own logic is almost untouched by that — it still reserves before
/// starting and slashes on fault — because the collateral source was always behind an interface.
/// What changes is that a release now also *reports the outcome*, since a credit line has a
/// memory and a bond did not.
///
/// ERC-8004 Validation Registry wiring (the requestHash gate in createJob, and the
/// validationResponse attestation calls) is unchanged — see isValidationRequestValid and
/// _attestValidation.
contract JobEscrow {
    ...
    struct Job {
        ...
        // payout through an NFT sale after delivery. Note this cuts the opposite way from a
        // credit *repayment*, which anyone may make on an agent's behalf (see CreditLine.repay):
        // money owed to the pool should be as easy to pay as possible, while money owed to a
        // specific counterparty must go exactly where it was promised.
        address sellerPayoutAddress;
        uint256 amount;
        // amount * coverageRatioBps / 10000 at creation time — fixed for this job's lifetime.
        ...
        // rationale as reservedCredit above — and note CreditLine.repaymentGracePeriod
        // deliberately does the opposite, applying retroactively, because it is a live risk
        // dial over the whole loan book rather than a term of one job. Here a later
    ...
    // Settable exactly once post-deploy (see setCreditLine) — resolves the circular
    // constructor dependency: JobEscrow needs CreditLine's address, but CreditLine's
    // JOB_ESCROW is immutable and needs JobEscrow's address. JobEscrow deploys first with
    // this unset; CreditLine deploys second with JobEscrow's address baked in as immutable;
    // then setCreditLine() is called once to complete the wiring. CreditPool follows the same
    // dance with its own setCreditLine().
    ...
    // Risk parameter: how much credit a job must have committed behind it, as a fraction of
    // the job amount in basis points (2000 = 20%). Under Tripwire this was the seller's own
    // posted stake; now it is the size of the pool's exposure if this job goes bad, which is
    // why it stays a protocol-level dial rather than something a buyer or seller picks.
    uint256 public coverageRatioBps = 2000;
    // Hard ceiling on coverageRatioBps (10_000 = 100%): bounds a careless or compromised owner
    ...
    // configuration: createJob/release/resolveDispute/claimTimeout all guard their CreditLine
    // calls behind `reservedCredit > 0`, so a 0%-ratio job runs its full lifecycle without ever
    // touching CreditLine (which would otherwise reject the zero-amount calls) — same
    // reasoning as CreditLine's MAX_REPAYMENT_GRACE having no floor. Worth knowing: such a job
    // also earns the seller no completion history, since nothing was ever committed.
    uint256 public constant MAX_COVERAGE_RATIO_BPS = 10_000;
    ...
    // Mirrors CreditLine.MAX_REPAYMENT_GRACE exactly, same rationale: bounds what a
    ...
    // pattern this very codebase's own tests use for ASSET allowances
    ...
    // safety bound, not a risk/business parameter like coverageRatioBps or responseWindow.
    ...
    error CoverageRatioTooHigh(uint256 requested, uint256 max);
    ...
    /// @dev Every argument is checked, and `arbiter_` is the reason why. ARBITER is immutable,
    /// and it is the only address that can resolve a dispute — deploying with a zero arbiter
    /// would leave every disputed job permanently unresolvable with no way to fix it short of
    /// redeploying, and nothing later in the lifecycle would surface the mistake until the
    /// first dispute.
    constructor(address asset_, address identityRegistry_, address validationRegistry_, address arbiter_) {
        if (
            asset_ == address(0) || identityRegistry_ == address(0) || validationRegistry_ == address(0)
                || arbiter_ == address(0)
        ) {
            revert ZeroAddress();
        }
    ...
    function setCreditLine(address creditLine_) external onlyOwner {
        if (address(creditLine) != address(0)) revert CreditLineAlreadySet();
        // Without this, setCreditLine(0) is accepted, wires nothing, and leaves the one-shot
        // guard unlatched — a silent no-op that looks like a completed deploy step. CreditPool's
        // equivalent setter already checked for this; the two now match.
        if (creditLine_ == address(0)) revert ZeroAddress();
        creditLine = ICreditLine(creditLine_);
        emit CreditLineSet(creditLine_);
    }

    /// @notice Buyer opens a job: validates amount/deadline, reserves the seller's bond,
    /// pulls `amount` ASSET into escrow. Reverts via CreditLine.reserve() if the seller is
    /// unverified, over its per-job cap, delinquent, or out of headroom for coverageRatioBps of
    /// `amount` — no duplicate eligibility check here, CreditLine is the source of truth for
    ...
    /// the existence check, same pattern as ApassRegistry.attest.
        ...
        returns (uint256 jobId)
    {
        ...
        if (address(creditLine) == address(0)) revert CreditLineNotSet();
        ...
        // released, or slashed for it, regardless of later coverageRatioBps changes. Can be
        // zero when coverageRatioBps is 0 (see the guard below) — a deliberately supported
        // demo/testing configuration, not an edge case to reject.
        uint256 reservedCredit = (amount * coverageRatioBps) / 10_000;
        ...
        emit JobCreated(jobId, msg.sender, sellerAgentId, amount, reservedCredit, completionDeadline);

        // Interactions last. If the seller's free bond can't cover reservedCredit,
        // creditLine.reserve() reverts and unwinds the job creation atomically — no separate
        // ratio check needed here, CreditLine is the source of truth for its own limits.
        // Skipped entirely when reservedCredit is zero (coverageRatioBps == 0): CreditLine
        // rejects zero-amount calls, and there is genuinely nothing to reserve.
        if (reservedCredit > 0) {
            creditLine.reserve(sellerAgentId, reservedCredit);
        }
    ...
    function release(uint256 jobId) external onlyBuyer(jobId) {
        ...
        // Skipped when reservedCredit is zero (coverageRatioBps was 0 at creation) — nothing
        // was ever reserved, and CreditLine rejects zero-amount calls.
        // countsAsCompletion = true: the buyer actively confirmed delivery, which is the
        // strongest completion signal available and the one that should grow the seller's line.
        if (job.reservedCredit > 0) {
            creditLine.releaseReservation(job.sellerAgentId, job.reservedCredit, true);
        }
        ASSET.safeTransfer(job.sellerPayoutAddress, job.amount);
    ...
    function resolveDispute(uint256 jobId, bool sellerAtFault) external onlyArbiter {
        ...
        if (sellerAtFault) {
            ...
            // the credit pool, plus the escrowed amount refunded from JobEscrow's own
            // holdings — real compensation on top of just getting their own money back.
            // Skipped when reservedCredit is zero — see release()'s equivalent comment.
            if (job.reservedCredit > 0) {
                creditLine.slash(job.sellerAgentId, job.reservedCredit, job.buyer);
            }
            ASSET.safeTransfer(job.buyer, job.amount);
            ...
            // countsAsCompletion = true, same as release(): an arbiter examined the job and
            // found for the seller, so it counts exactly as much as an uncontested delivery.
            // Deliberately not discounted for having been disputed — penalising a seller for
            // winning a dispute would make raising one a free way to suppress a rival's limit.
            if (job.reservedCredit > 0) {
                creditLine.releaseReservation(job.sellerAgentId, job.reservedCredit, true);
            }
            ASSET.safeTransfer(job.sellerPayoutAddress, job.amount);
    ...
    function claimTimeout(uint256 jobId) external {
        ...
        // countsAsCompletion = false. The seller is paid — a buyer must not be able to grief
        // them by going silent — but nobody confirmed the work, so it must not grow the credit
        // line. Otherwise the cheapest way to farm a large limit would be to run jobs with a
        // buyer wallet you control and simply never respond. Mirrors the response=50
        // "indeterminate" attestation below: paid, not vouched for.
        if (job.reservedCredit > 0) {
            creditLine.releaseReservation(job.sellerAgentId, job.reservedCredit, false);
        }
        ASSET.safeTransfer(job.sellerPayoutAddress, job.amount);
    ...
    /// already-active jobs keep their snapshotted reservedCredit (invariant 1). Capped at
    /// MAX_COVERAGE_RATIO_BPS.
    function setCoverageRatioBps(uint256 newRatioBps) external onlyOwner {
        if (newRatioBps > MAX_COVERAGE_RATIO_BPS) {
            revert CoverageRatioTooHigh(newRatioBps, MAX_COVERAGE_RATIO_BPS);
        }
        emit CoverageRatioBpsUpdated(coverageRatioBps, newRatioBps);
        coverageRatioBps = newRatioBps;
    }
